...
void main ()
{
    ...
    float depth1 = nom / (denom1 - texture2DRect (tex_depth, gl_TexCoord [0].st + vec2 (0.0,   1.0)).x * denom2);
    float depth2 = nom / (denom1 - texture2DRect (tex_depth, gl_TexCoord [0].st + vec2 (0.0, - 1.0)).x * denom2);
    ...
    gl_FragColor   += texture2DRect (tex, gl_TexCoord [0].st + vec2 (0.0,   1.0)) * f1;
    gl_FragColor   += texture2DRect (tex, gl_TexCoord [0].st + vec2 (0.0, - 1.0)) * f2;
...
//  float factor = min (1.0, 1.0 * log (1.0 + (abs (depth - depth1) + abs (depth - depth2)) * 100.0));
    ...
    gl_FragColor += texture2DRect (tex, gl_TexCoord [0].st + vec2 (0.0,   1.0)) * (4.0 / 14.0);
    gl_FragColor += texture2DRect (tex, gl_TexCoord [0].st + vec2 (0.0, - 1.0)) * (4.0 / 14.0);
    gl_FragColor += texture2DRect (tex, gl_TexCoord [0].st + vec2 (0.0,   2.0)) * (1.0 / 14.0);
    gl_FragColor += texture2DRect (tex, gl_TexCoord [0].st + vec2 (0.0, - 2.0)) * (1.0 / 14.0);
