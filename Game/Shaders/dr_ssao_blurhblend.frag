...
void main ()
{
    ...
    sample = texture2DRect (tex_G2, gl_TexCoord [1].st);    depths = 1.0 / (1.0 + 65535.0 * abs (depth - sample.z));
    ao += texture2DRect (tex_vblur, gl_TexCoord [1].pq).r * depths;    sum += depths;

    sample = texture2DRect (tex_G2, gl_TexCoord [2].st);    depths = 1.0 / (1.0 + 65535.0 * abs (depth - sample.z));
    ao += texture2DRect (tex_vblur, gl_TexCoord [2].pq).r * depths;    sum += depths;

    sample = texture2DRect (tex_G2, gl_TexCoord [3].st);    depths = 1.0 / (1.0 + 65535.0 * abs (depth - sample.z));
    ao += texture2DRect (tex_vblur, gl_TexCoord [3].pq).r * depths;    sum += depths;

    sample = texture2DRect (tex_G2, gl_TexCoord [4].st);    depths = 1.0 / (1.0 + 65535.0 * abs (depth - sample.z));
